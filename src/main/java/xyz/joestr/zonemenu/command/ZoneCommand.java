	...
	public boolean onCommand(CommandSender sender, Command command, String label, final String[] args) {
		...
		if (sender instanceof Player) {
			...
			if (player.hasPermission("zonemenu.*")) {
				...
				// TODO: Implementig /zone flag <Flag> <Flag value ...>
				// -> /zone flag <Flag> <Flag value...>
				/*
				if ((args[0].equalsIgnoreCase("flag")) && (args.length > 1) && (args.length < 4)) {
					...
					// flags[1].parseInput(arg0, arg1, arg2)
				...
				if ((args[0].equalsIgnoreCase("info")) && (args.length < 2)) {
					...
					player.sendMessage(
							this.plugin.colorCode("&", (String) this.plugin.config.Map().get("zone_info_members"))
									+ RegionMember.toPlayersString(this.plugin.worldguardplugin.getProfileCache())
											.replace("*", ""));
					player.sendMessage(
